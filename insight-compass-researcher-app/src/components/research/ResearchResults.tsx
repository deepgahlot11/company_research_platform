...
export const ResearchResults: React.FC<ResearchResultsProps> = ({ result, isLoading, companyName }) => {
  ...
  const exportToPDF = () => {
    ...
    // Save the PDF
    doc.save(`${data.company.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_research.pdf`);
  };
  ...
  if (!result) {
    return (
      <Card className="h-full">
        ...
        <CardContent>
          <div className="flex items-center justify-center h-64">
            <p className="text-muted-foreground">Enter a company name to start researching</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  if (!result.success) {
    return (
      <Card className="h-full">
        ...
        <CardContent>
          <div className="flex items-center justify-center h-64">
            <p className="text-destructive">{result.message || 'Failed to research company'}</p>
          </div>
        </CardContent>
      </Card>
    );
